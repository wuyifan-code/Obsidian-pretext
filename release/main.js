...
var CodeMirrorExtension_exports = {};
__export(CodeMirrorExtension_exports, {
  createPretextCodeMirrorExtension: () => createPretextCodeMirrorExtension
});
function createPretextCodeMirrorExtension(pretextManager) {
...
var import_view;
var init_CodeMirrorExtension = __esm({
  "src/hooks/CodeMirrorExtension.ts"() {
    "use strict";
...
var MeasurementCache = class {
  ...
  get(text, fontFamily, fontSize, fontWeight, maxWidth, lineHeight) {
  ...
  set(text, fontFamily, fontSize, fontWeight, maxWidth, lineHeight, value) {
...
function createMarkdownPostProcessor(pretextManager, cache) {
  return (element, context) => {
    if (!pretextManager.isReady()) {
      return;
    }
    for (const selector of HEAVY_SELECTORS) {
      const heavyEls = element.querySelectorAll(selector);
      heavyEls.forEach((el) => processHeavyElement(el, pretextManager, cache));
    }
  };
}
function processHeavyElement(el, pretextManager, cache) {
...
// main.ts
var ObsidianPretextPlugin = class extends import_obsidian.Plugin {
  async onload() {
    console.log("[Pretext Optimizer] Loading plugin...");
    this.loadPretextBundle();
    this.measurementCache = new MeasurementCache(1e3);
...
`;
  ...
  tryRegisterCodeMirrorExtension() {
    if (typeof this.registerCodeMirrorExtension === "function") {
      try {
        Promise.resolve().then(() => (init_CodeMirrorExtension(), CodeMirrorExtension_exports)).then(({ createPretextCodeMirrorExtension: createPretextCodeMirrorExtension2 }) => {
          const extension = createPretextCodeMirrorExtension2(this.pretextManager);
